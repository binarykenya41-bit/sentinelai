...
async function markRunning(jobId: string, sandboxId: string) {
...
async function markCompleted(jobId: string, resultId: string, success: boolean) {
  await supabase.from("simulation_queue").update({
    status: "completed",
    result_id: resultId,
...
async function escalateVulnerability(vulnId: string, cveId: string) {
  await supabase
 ...
 */
export async function buildSimulationQueue(): Promise<number> {
  if (!AUTO_SIM_ENABLED) {
  ...
  for (const vuln of vulns ?? []) {
    ...
    if (!module && vuln.mitre_techniques?.length) {
      for (const tid of vuln.mitre_techniques) {
        ...
        if (matches.length) { module = matches[0]; break }
      }
    }
    if (!module) continue
 ...
 */
export async function dispatchSimulationQueue(): Promise<AutoSimResult> {
  ...
  const results: AutoSimResult["results"] = []
...
export async function runAutoSimCycle(): Promise<{ queued: number; dispatch: AutoSimResult }> {
...
export async function emergencyStop(): Promise<{ killed: string[]; cleared: number }> {
