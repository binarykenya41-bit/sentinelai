...
export interface SandboxConfig {
  ...
  timeout_ms: number
  cpu_quota?: number    // Docker --cpu-quota (e.g. 50000 = 50%)
  memory_mb?: number    // Docker --memory in MB
}
...
export async function ensureSandboxNetwork(): Promise<void> {
  try {
    ...
    await execAsync(
      `docker network create --driver bridge --internal ${SANDBOX_NETWORK}`
    )
  }
}
  ...
  try {
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) =>
        setTimeout(() => reject(new Error("SANDBOX_TIMEOUT")), timeoutMs)
      ),
    ])
    ...
    if (error.message === "SANDBOX_TIMEOUT") {
      ...
      // Kill the container
      try { await execAsync(`docker kill ${sandboxId}`) } catch { /* already gone */ }
    } else {
...
export async function killSandbox(sandboxId: string): Promise<void> {
  await execAsync(`docker kill ${sandboxId}`).catch(() => {/* already stopped */})
}
...
export async function listRunningSandboxes(): Promise<string[]> {
  const { stdout } = await execAsync(
    `docker ps --filter "name=sb-" --format "{{.Names}}" 2>/dev/null || echo ""`
  )
