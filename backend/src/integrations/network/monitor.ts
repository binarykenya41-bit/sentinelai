...
export interface NetworkFlow {
  ...
  first_seen: string
  last_seen: string
  score: number          // ntopng anomaly score (0–100)
}
...
async function ntopngAuth(): Promise<string> {
  const { data } = await axios.post(
    `${NTOPNG_BASE}/lua/rest/v2/login.lua`,
    new URLSearchParams({ username: NTOPNG_USER, password: NTOPNG_PASS }),
    { timeout: 8_000 }
  )
  return data?.rsp?.session_id ?? ""
}

export async function getTopFlows(ifaceId = 0, limit = 50): Promise<NetworkFlow[]> {
  try {
    ...
    return (data?.rsp?.flows ?? []).map((f: Record<string, unknown>) => ({
      client_ip: f["cli.ip"] as string,
      server_ip: f["srv.ip"] as string,
      client_port: f["cli.port"] as number,
      server_port: f["srv.port"] as number,
      protocol: f["proto.l4"] as string,
      bytes_sent: f["bytes.sent"] as number,
      bytes_rcvd: f["bytes.rcvd"] as number,
      duration_ms: (f["duration"] as number) * 1000,
      first_seen: new Date((f["seen.first"] as number) * 1000).toISOString(),
      last_seen: new Date((f["seen.last"] as number) * 1000).toISOString(),
...
export async function getInterfaces(): Promise<NetworkInterface[]> {
  try {
    ...
    return (data?.rsp ?? []).map((i: Record<string, unknown>) => ({
      id: i.id as number,
      name: i.ifname as string,
      speed: i.speed as number,
      packets: (i.stats as Record<string, number>)?.packets ?? 0,
      bytes: (i.stats as Record<string, number>)?.bytes ?? 0,
      drops: (i.stats as Record<string, number>)?.drops ?? 0,
    }))
  } catch {
    return []
  }
...
export async function getRecentSuricataAlerts(maxLines = 200): Promise<SuricataAlert[]> {
  ...
  try {
    ...
    for await (const line of rl) {
      ...
      if (alerts.length >= maxLines) break
    }
...
async function zabbixLogin(): Promise<string> {
  if (_zabbixToken) return _zabbixToken
...
const ZABBIX_SEVERITY: Record<number, ZabbixProblem["severity"]> = {
  0: "not_classified", 1: "info", 2: "warning", 3: "average", 4: "high", 5: "disaster",
}

export async function getZabbixProblems(minSeverity = 2): Promise<ZabbixProblem[]> {
  try {
    ...
    return (data.result ?? []).map((p: Record<string, unknown>) => ({
      ...
      host: ((p.hosts as Record<string, string>[])[0])?.host ?? "unknown",
      name: p.name as string,
      severity: ZABBIX_SEVERITY[parseInt(p.severity as string)] ?? "not_classified",
      acknowledged: (p.acknowledged as string) === "1",
      clock: new Date(parseInt(p.clock as string) * 1000).toISOString(),
    }))
  } catch {
    return []
  }
...
export async function getNetworkPosture() {
  ...
  return {
    ...
    posture_score: Math.max(
      0,
      100 - criticalAlerts.length * 10 - highZabbix.length * 5 - flows.filter((f) => f.score > 80).length * 15
    ),
  }
}
