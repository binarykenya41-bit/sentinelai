...
export async function incidentsRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { status?: string; severity?: string; limit?: string; offset?: string } }>(
    "/api/incidents", async (req, reply) => {
      ...
      if (req.query.offset) q = q.range(parseInt(req.query.offset), parseInt(req.query.offset) + parseInt(req.query.limit ?? "50") - 1)
  ...
  app.get("/api/incidents/stats", async (req, reply) => {
    ...
    const mttrValues = resolved.map(r => r.mttr_hours).filter(Boolean) as number[]
  ...
  app.get<{ Params: { id: string } }>("/api/incidents/:id", async (req, reply) => {
    ...
    if (error) return reply.status(404).send({ error: "Incident not found" })
  ...
  app.post<{ Body: { title: string; severity: string; category?: string; assigned_to?: string; affected_assets?: string[]; description?: string } }>(
    "/api/incidents", async (req, reply) => {
      const industry = req.headers['x-industry'] as string | undefined
  ...
  app.patch<{ Params: { id: string }; Body: { status?: string; progress?: number; assigned_to?: string } }>(
    "/api/incidents/:id", async (req, reply) => {
