...
import {
  getTopFlows,
  getInterfaces,
  getRecentSuricataAlerts,
  getZabbixProblems,
  getNetworkPosture,
} from "../integrations/network/monitor.js"
...
export async function networkRoutes(app: FastifyInstance) {
  ...
  app.get<{ Querystring: { iface?: string; limit?: string } }>(
    "/api/network/flows",
    async (req, reply) => {
      const flows = await getTopFlows(
        parseInt(req.query.iface ?? "0"),
  ...
  app.get<{ Querystring: { min_severity?: string } }>(
