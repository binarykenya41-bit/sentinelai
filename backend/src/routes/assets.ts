...
export async function assetsRoutes(app: FastifyInstance) {
  ...
  app.get<{
    ...
    if (req.query.patch_status) query = query.eq("patch_status", req.query.patch_status)
  ...
  app.get("/api/assets/stats", async (_req, reply) => {
    ...
    const byPatchStatus: Record<string, number> = {}
  ...
  app.get<{ Params: { id: string } }>("/api/assets/:id", async (req, reply) => {
    ...
    if (error) return reply.status(404).send({ error: "Asset not found" })
  ...
  app.patch<{
    Params: { id: string }
    Body: { criticality?: string; patch_status?: string; tags?: string[] }
  }>("/api/assets/:id", async (req, reply) => {
    const { data, error } = await supabase
      .from("assets")
