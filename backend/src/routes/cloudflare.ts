...
export async function cloudflareRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { zoneId: string } }>(
    "/api/cloudflare/zones/:zoneId/waf",
    async (req, reply) => {
      const { zoneId } = req.params
  ...
  app.get<{ Params: { zoneId: string }; Querystring: { since?: string; limit?: string } }>(
    "/api/cloudflare/zones/:zoneId/logs",
    async (req, reply) => {
      const { zoneId } = req.params
  ...
  app.get<{ Params: { zoneId: string } }>(
    "/api/cloudflare/zones/:zoneId/summary",
    async (req, reply) => {
      const { zoneId } = req.params
