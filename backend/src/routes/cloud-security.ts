...
export async function cloudSecurityRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { provider?: string; severity?: string; status?: string; limit?: string; offset?: string } }>(
    "/api/cloud-security", async (req, reply) => {
  ...
  app.get("/api/cloud-security/stats", async (req, reply) => {
    ...
    const by_provider = providers.reduce((acc: Record<string, { total: number; open: number; score: number }>, p) => {
    ...
    return reply.send({
      ...
      by_provider: rows.reduce((acc: Record<string, number>, r) => { acc[r.provider] = (acc[r.provider] ?? 0) + 1; return acc }, {}),
      provider_scores: by_provider,
    })
  })

  app.get<{ Params: { id: string } }>("/api/cloud-security/:id", async (req, reply) => {
  ...
  app.patch<{ Params: { id: string }; Body: { status?: string } }>(
    "/api/cloud-security/:id", async (req, reply) => {
