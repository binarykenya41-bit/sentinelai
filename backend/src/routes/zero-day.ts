...
export async function zeroDayRoutes(app: FastifyInstance) {
  app.get<{ Querystring: { status?: string; severity?: string; limit?: string } }>(
    "/api/zero-day", async (req, reply) => {
  ...
  app.get("/api/zero-day/stats", async (req, reply) => {
    ...
    return reply.send({
      ...
      by_maturity: rows.reduce((acc: Record<string, number>, r) => { acc[r.exploit_maturity ?? "unknown"] = (acc[r.exploit_maturity ?? "unknown"] ?? 0) + 1; return acc }, {}),
    })
  })

  app.get<{ Params: { id: string } }>("/api/zero-day/:id", async (req, reply) => {
    const { data, error } = await supabase.from("zero_days").select("*").eq("zd_id", req.params.id).single()
    if (error) return reply.status(404).send({ error: "Zero-day not found" })
