...
export async function getSecurityEvents(zoneId, since = new Date(Date.now() - 24 * 60 * 60 * 1000).toISOString(), limit = 100) {
    try {
        const raw = await cfFetch(`/zones/${zoneId}/security/events?since=${encodeURIComponent(since)}&limit=${limit}`);
        return Array.isArray(raw) ? raw : [];
    }
    catch (err) {
        console.warn("[cloudflare/logs] security events unavailable:", err.message);
        return [];
    }
}
// Aggregate firewall event stats for the Sentinel dashboard
export function aggregateFirewallStats(events) {
    const byAction = {};
    const byCountry = {};
    const bySource = {};
    const topIps = {};
