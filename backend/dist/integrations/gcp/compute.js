...
export async function listAllInstances() {
    ...
    for (const [, zoneData] of Object.entries(data.items ?? {})) {
        for (const vm of zoneData.instances ?? []) {
            ...
            const externalIp = networkInterface?.accessConfigs?.[0]?.natIP ?? null;
...
export async function syncGcpInstancesToAssets(orgId) {
    ...
    for (const vm of instances) {
        const ip = [];
        if (vm.internal_ip)
            ip.push(vm.internal_ip);
        if (vm.external_ip)
            ip.push(vm.external_ip);
