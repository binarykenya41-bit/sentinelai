...
export async function handleWorkflowRunEvent(payload) {
    ...
    if (!branchName?.startsWith("sentinel/"))
        return;
    const ciStatus = workflow_run.status === "completed"
    ...
    if (error)
        console.error("[github-webhook] patch_records update failed:", error.message);
}
...
export async function handlePullRequestEvent(payload) {
    ...
    if (!headRef?.startsWith("sentinel/"))
        return;
    const mergeStatus = action === "closed" && pull_request.merged
        ? "merged"
        : action === "closed"
            ? "blocked"
    ...
    if (error)
        console.error("[github-webhook] pull_request update failed:", error.message);
}
