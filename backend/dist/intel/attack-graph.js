...
import { listAllTechniques, getTechnique, getTactics, mapCweToTechniques, } from "./mitre.js";
...
function tacticRiskColor(tactic) {
 ...
 */
export async function buildCveAttackGraph(cveIds, assetIds = []) {
    const nodes = [];
    const edges = [];
    const seenNodes = new Set();
    const seenEdges = new Set();
    const sources = ["NVD", "EPSS", "CISA KEV", "MITRE ATT&CK STIX"];
    ...
    const cveData = await Promise.all(cveIds.map(async (cveId) => {
    ...
    for (const d of cveData) {
        ...
        const risk = Math.min(100, Math.round(cvss * 4 + epssScore * 30 + (isKev ? 20 : 0) + (exploitAvail ? 10 : 0)));
        addNode({
            id: d.cveId,
            type: "cve",
            label: d.cveId,
    ...
    const techniqueSet = new Map();
    for (const d of cveData) {
        if (!d.nvd?.cwe_ids?.length)
            continue;
    ...
    const techniquesByTactic = new Map();
    for (const tech of techniqueSet.values()) {
        for (const phase of tech.tactic_phases) {
            if (!techniquesByTactic.has(phase))
                techniquesByTactic.set(phase, []);
    ...
    const usedTactics = new Set(nodes
 ...
 */
export async function buildFullAttackMatrix() {
