        ...
        known_cves: [
            {
                ...
                description: "GitLab allows account takeover by sending a password reset link to an attacker-controlled unverified email address. No authentication required.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Authenticated GitLab users can write files to arbitrary server paths via a crafted project import archive, enabling RCE via config file overwrite.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Improper authorization allows users to run Slack/Mattermost slash commands as other users by exploiting integration webhook trust.",
        ...
        known_cves: [
            {
                ...
                kev: false,
                exploit_available: true,
                description: "The WPML plugin allows authenticated users with Contributor+ permissions to execute arbitrary PHP via TWIG template injection in shortcodes.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "WordPress avatar block doesn't properly sanitize HTML attributes, allowing stored XSS from any authenticated user that affects all visitors.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Directory traversal via the wp_lang parameter in load-scripts.php allows unauthenticated users to read files outside the web root.",
        ...
        known_cves: [
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Frappe (ERPNext foundation) contains unsanitized user input in dynamically generated SQL queries in the report-builder, enabling full database dump.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "ERPNext user profile fields (full name, bio) are not properly sanitized, allowing stored XSS that fires for all users who view the profile — enables session hijacking.",
                mitre_techniques: ["T1059.007", "T1185"],
                tactic: "Collection",
        ...
        known_cves: [
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Keycloak fails to validate path components in redirect_uri during OAuth2 authorization. Attackers can bypass allowed redirect_uri restrictions and steal authorization codes.",
            ...
            },
            {
                ...
                description: "Low-privilege Keycloak users can access admin-only REST API endpoints by directly calling undisclosed paths, enabling role and group manipulation.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Keycloak open redirect when the redirect_uri parameter is not fully validated, allowing phishing via SSO login flow redirection to attacker domains.",
        ...
        known_cves: [
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Non-privileged PostgreSQL users can escalate to superuser by exploiting a security definer function vulnerability via crafted ALTER TABLE ... SET SCHEMA commands.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: false,
                description: "Memory safety issues in range and multirange type arithmetic functions can be exploited by authenticated users to crash the server or potentially execute arbitrary code.",
        ...
        known_cves: [
            {
                ...
                kev: true,
                exploit_available: true,
                description: "Grafana plugin static file serving is vulnerable to path traversal. Unauthenticated attackers can read any file accessible by the Grafana process, including /etc/passwd and private keys.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Grafana forwards auth JWT tokens via URL parameters when accessing datasource APIs, allowing tokens to leak into server access logs and referrer headers.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Cross-site request forgery allows remote attackers to perform state-changing operations including admin account creation and org-level privilege escalation.",
                mitre_techniques: ["T1078", "T1548"],
                tactic: "Privilege Escalation",
        ...
        known_cves: [
            {
                ...
                kev: false,
                exploit_available: true,
                description: "Prometheus alertmanager stores alert labels without sanitization, enabling stored XSS that fires in the alertmanager UI for any user who views the alert.",
            ...
            },
            {
                ...
                kev: false,
                exploit_available: false,
                description: "Prometheus client_golang HTTP instrumentation middleware is vulnerable to regex denial of service via crafted HTTP method strings with deeply nested backtracking.",
...
function httpProbe(host, port, path, useSsl = false, timeoutMs = 5000) {
    return new Promise((resolve) => {
...
async function scanService(def, host, port) {
    const start = Date.now();
    let reachable = false;
    let detected_version = null;
    const fingerprint = {};
    if (def.protocol === "tcp") {
        ...
        if (result) {
            ...
            if (result.headers["server"])
                fingerprint["server"] = result.headers["server"];
            if (result.headers["x-powered-by"])
                fingerprint["x-powered-by"] = result.headers["x-powered-by"];
            if (result.headers["x-generator"])
                fingerprint["x-generator"] = result.headers["x-generator"];
            // WordPress detection
            if (def.id === "wordpress" && result.body.includes("wp-"))
                fingerprint["cms"] = "WordPress";
            // Grafana detection
            if (def.id === "grafana" && result.body.includes("grafana"))
                fingerprint["app"] = "Grafana";
            // Keycloak detection
    ...
    const risk_score = vulnerabilities.length
        ? Math.round(vulnerabilities.reduce((sum, v) => {
    ...
    const highest_severity = vulnerabilities.length
...
function buildInfraAttackGraph(results) {
    const nodes = [];
    const edges = [];
    ...
    for (const result of results) {
        ...
        const initCves = result.vulnerabilities.filter((v) => ["Initial Access", "Execution"].includes(v.tactic));
...
export async function infraScanRoutes(app) {
    ...
    app.post("/api/infra-scan/run", async (req, reply) => {
        ...
        (async () => {
            try {
                const results = await Promise.all(normalizedTargets.map((t) => {
    ...
    app.get("/api/infra-scan/:scan_id", async (req, reply) => {
...
//# sourceMappingURL=infra-scan.js.map