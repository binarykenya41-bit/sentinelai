...
export async function syncRoutes(app) {
    ...
    app.post("/api/sync/nvd", async (req, reply) => {
    ...
    app.post("/api/sync/kev", async (req, reply) => {
    ...
    app.get("/api/sync/queue", async (req, reply) => {
        let query = supabase
    ...
    app.get("/api/sync/threat-feed", async (req, reply) => {
        let query = supabase
            .from("threat_feed")
            .select("*")
        ...
        if (req.query.exploit === "true")
            query = query.eq("exploit_available", true);
        if (req.query.min_cvss)
            query = query.gte("cvss_v3", parseFloat(req.query.min_cvss));
        if (req.query.min_priority)
            query = query.gte("priority_score", parseInt(req.query.min_priority));
    ...
    app.get("/api/sync/threat-feed/:cveId", async (req, reply) => {
