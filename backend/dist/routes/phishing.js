...
export async function phishingRoutes(app) {
    app.get("/api/phishing", async (req, reply) => {
        ...
        if (req.query.department)
            q = q.eq("target_department", req.query.department);
    ...
    app.get("/api/phishing/stats", async (req, reply) => {
        ...
        for (const r of completed) {
            ...
            if (!deptMap[dept])
                deptMap[dept] = { recipients: 0, clicked: 0, submitted: 0 };
