...
export async function redTeamRoutes(app) {
    app.get("/api/red-team", async (req, reply) => {
    ...
    app.get("/api/red-team/stats", async (req, reply) => {
        ...
        return reply.send({
            ...
            by_kill_chain: rows.reduce((acc, r) => { if (r.kill_chain_stage) {
                acc[r.kill_chain_stage] = (acc[r.kill_chain_stage] ?? 0) + 1;
            } return acc; }, {}),
        });
    });
    app.get("/api/red-team/:id", async (req, reply) => {
        const { data, error } = await supabase.from("red_team_campaigns").select("*").eq("campaign_id", req.params.id).single();
        if (error)
            return reply.status(404).send({ error: "Campaign not found" });
        return reply.send(data);
    });
    app.post("/api/red-team", async (req, reply) => {
        const industry = req.headers['x-industry'];
    ...
    app.patch("/api/red-team/:id", async (req, reply) => {
