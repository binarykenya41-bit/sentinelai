...
export async function integrationRoutes(app) {
    ...
    app.get("/api/integrations", async (req, reply) => {
        ...
        if (req.query.org_id)
            query = query.eq("org_id", req.query.org_id);
    ...
    app.post("/api/integrations", async (req, reply) => {
        ...
        const { data, error } = await supabase
            .from("integrations")
            .upsert({ org_id, category, tool_id, name, config: config ?? null, status: "pending", updated_at: new Date().toISOString() }, { onConflict: "org_id,tool_id" })
            .select()
            .single();
        if (error)
            return reply.status(500).send({ error: error.message });
    ...
    app.patch("/api/integrations/:id/status", async (req, reply) => {
