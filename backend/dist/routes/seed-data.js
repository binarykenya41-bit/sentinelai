...
function buildComplianceControls(vulnIds) {
    ...
    function computeScore(controls) {
...
export async function seedDataRoutes(app) {
    ...
    app.post("/api/seed/compliance", async (_req, reply) => {
        ...
        for (const report of reports) {
            ...
            if (!error)
                inserted.push(report.framework);
        }
        return reply.send({
            ok: true,
    ...
    app.post("/api/seed/threat-intel", async (_req, reply) => {
        ...
        for (const entry of LOGISTICS_THREAT_FEED) {
            ...
            if (error)
                results.threat_feed.errors.push(`${entry.cve_id}: ${error.message}`);
            else
                results.threat_feed.inserted++;
        }
        // ── Dark Web Findings ──────────────────────────────────────────────────────
        ...
        for (const finding of DW_FINDINGS) {
            ...
            if (error)
                results.dark_web_findings.errors.push(`${finding.title.slice(0, 40)}: ${error.message}`);
            else
                results.dark_web_findings.inserted++;
        }
        // ── EDR Alerts ─────────────────────────────────────────────────────────────
        ...
        for (const alert of EDR_ALERTS) {
            ...
            if (error)
                results.edr_alerts.errors.push(`${alert.title.slice(0, 40)}: ${error.message}`);
            else
                results.edr_alerts.inserted++;
        }
        // ── Malware Samples ────────────────────────────────────────────────────────
        ...
        for (const sample of MALWARE_SAMPLES) {
            ...
            if (error)
                results.malware_samples.errors.push(`${sample.file_name}: ${error.message}`);
            else
                results.malware_samples.inserted++;
        }
        // ── Phishing Campaigns ─────────────────────────────────────────────────────
        ...
        for (const campaign of PHISHING_CAMPAIGNS) {
            ...
            if (error)
                results.phishing_campaigns.errors.push(`${campaign.name.slice(0, 40)}: ${error.message}`);
            else
                results.phishing_campaigns.inserted++;
        }
        return reply.send({
            ok: true,
            results,
