...
export async function intelRoutes(app) {
    // ─── CVE / NVD ──────────────────────────────────────────────────────────────
    // GET /api/intel/cve/:id — fetch single CVE from NVD
    app.get("/api/intel/cve/:id", async (req, reply) => {
    ...
    app.get("/api/intel/cve/:id/enriched", async (req, reply) => {
    ...
    app.get("/api/intel/cves/search", async (req, reply) => {
        const result = await searchCves({
            keyword: req.query.keyword,
            cweId: req.query.cweId,
            cvssV3SeverityMin: req.query.severity,
    ...
    app.post("/api/intel/cves/batch-enrich", async (req, reply) => {
        const { cve_ids } = req.body;
        if (!cve_ids?.length)
            return reply.status(400).send({ error: "cve_ids array is required" });
        if (cve_ids.length > 50)
            return reply.status(400).send({ error: "Max 50 CVEs per batch" });
    ...
    app.get("/api/intel/epss/:cveId", async (req, reply) => {
        const score = await fetchEpssScore(req.params.cveId.toUpperCase());
        if (!score)
            return reply.status(404).send({ error: "EPSS score not found" });
        return reply.send(score);
    });
    // GET /api/intel/epss/top — top CVEs by EPSS score
    app.get("/api/intel/epss/top", async (req, reply) => {
        const scores = await fetchTopEpss(parseInt(req.query.limit ?? "100"), parseFloat(req.query.min ?? "0.5"));
        return reply.send(scores);
    });
    // POST /api/intel/epss/batch — batch EPSS lookup
    app.post("/api/intel/epss/batch", async (req, reply) => {
        const { cve_ids } = req.body;
        if (!cve_ids?.length)
            return reply.status(400).send({ error: "cve_ids required" });
    ...
    app.get("/api/intel/kev/recent", async (req, reply) => {
    ...
    app.get("/api/intel/kev/:cveId", async (req, reply) => {
    ...
    app.get("/api/intel/mitre/techniques", async (req, reply) => {
        if (req.query.q) {
    ...
    app.get("/api/intel/mitre/techniques/:id", async (req, reply) => {
    ...
    app.post("/api/intel/mitre/map-cwe", async (req, reply) => {
        const { cwe_ids } = req.body;
        if (!cwe_ids?.length)
            return reply.status(400).send({ error: "cwe_ids required" });
    ...
    app.get("/api/intel/vuldb/cve/:id", async (req, reply) => {
        const entry = await fetchVulDbByCve(req.params.id.toUpperCase());
        if (!entry)
            return reply.status(404).send({ error: "Not found in VulDB" });
        return reply.send(entry);
    });
    // GET /api/intel/vuldb/recent — latest VulDB entries
    app.get("/api/intel/vuldb/recent", async (req, reply) => {
        const entries = await fetchVulDbRecent(parseInt(req.query.limit ?? "10"));
        return reply.send(entries);
    });
    // GET /api/intel/vuldb/search — search VulDB by keyword
    app.get("/api/intel/vuldb/search", async (req, reply) => {
        if (!req.query.q)
            return reply.status(400).send({ error: "q param required" });
    ...
    app.get("/api/intel/vuldb/exploitable", async (req, reply) => {
    ...
    app.post("/api/intel/vuldb/batch-exploit-status", async (req, reply) => {
        const { cve_ids } = req.body;
        if (!cve_ids?.length)
            return reply.status(400).send({ error: "cve_ids required" });
        if (cve_ids.length > 10)
            return reply.status(400).send({ error: "Max 10 CVEs (API credit limit)" });
