...
const SERVICES = {
    erpnext: {
        ...
        args: (host) => ["--target", `http://${host}:9000`, "--skip-brute"],
    },
    redis: {
        ...
        args: (host) => ["--host", host, "--port", "9379", "--check-cve-0543"],
    },
    kafka: {
        label: "Apache Kafka",
        host: "kafka",
        ...
        args: (_host) => ["--bootstrap", "kafka:9092", "--consume"],
    ...
    grafana: {
        ...
        args: (host) => ["--target", `http://${host}:9100`],
    },
    prometheus: {
        label: "Prometheus",
        host: "localhost",
        ...
        args: (host) => ["--target", `http://${host}:9191`],
    },
    postgresql: {
        label: "PostgreSQL",
        host: "localhost",
        ...
        args: (host) => ["--host", host, "--port", "9432", "--dbname", "shipments"],
    },
};
...
function runExploitScript(script, args, dockerNetwork) {
    return new Promise((resolve) => {
...
async function seedLogisticsData() {
    ...
    for (const a of allAssets ?? []) {
        assetMap[a.hostname] = a.asset_id;
    }
    ...
    const existingCves = new Set((existingVulns ?? []).map((v) => v.cve_id));
    ...
    return {
        ...
        asset_map: assetMap,
        vuln_ids: Object.fromEntries((finalVulns ?? []).map((v) => [v.cve_id, v.vuln_id])),
    };
}
// ─── Route registration ───────────────────────────────────────────────────────
export async function logisticsLabRoutes(app) {
    // GET /api/logistics/status — live port health for all 6 services
    app.get("/api/logistics/status", async (_req, reply) => {
        const checks = await Promise.all(Object.entries(SERVICES).map(async ([key, svc]) => {
    ...
    app.post("/api/logistics/exploit", async (req, reply) => {
        ...
        const { stdout, stderr, exitCode, durationMs } = await runExploitScript(svc.script, args, svc.dockerNetwork);
    ...
    app.get("/api/logistics/results", async (_req, reply) => {
        ...
        const { data: vulns } = await supabase
            ...
            .eq("scan_source", "logistics-lab");
        const vulnIds = (vulns ?? []).map((v) => v.vuln_id);
        let results = [];
        if (vulnIds.length > 0) {
        ...
        // Build vuln lookup
        const vulnMap = Object.fromEntries((vulns ?? []).map((v) => [v.vuln_id, v]));
        return reply.send({
            vulns: vulns ?? [],
            results: results.map((r) => ({
                ...r,
                vuln: vulnMap[r.vuln_id] ?? null,
            })),
...
//# sourceMappingURL=logistics-lab.js.map