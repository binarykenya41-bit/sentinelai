...
const LOGISTICS_PATCHES_DIR = join(dirname(fileURLToPath(import.meta.url)), "../../../containers/industries/logistics/patches");
// Map patch file → CVE for DB linkage
const PATCH_FILE_CVE_MAP = {
...
async function getOwnerRepo(octokit) {
...
export async function patchRoutes(app) {
    ...
    app.get("/api/patches", async (req, reply) => {
      ...
      `, { count: "exact" })
            ...
            .range(parseInt(req.query.offset ?? "0"), parseInt(req.query.offset ?? "0") + parseInt(req.query.limit ?? "50") - 1);
        if (req.query.ci_status)
            query = query.eq("ci_status", req.query.ci_status);
        if (req.query.merge_status)
            query = query.eq("merge_status", req.query.merge_status);
        if (req.query.resim_result)
            query = query.eq("resim_result", req.query.resim_result);
    ...
    app.get("/api/patches/:id", async (req, reply) => {
        const { data, error } = await supabase
            .from("patch_records")
            .select(`
        *,
      ...
      `)
            ...
            .single();
        if (error)
            return reply.status(404).send({ error: "Patch record not found" });
        return reply.send(data);
    });
    // POST /api/patches/generate — AI-generate patch + push branch + open PR
    app.post("/api/patches/generate", async (req, reply) => {
        const { vuln_id } = req.body ?? {};
        if (!vuln_id)
            return reply.status(400).send({ error: "vuln_id required" });
        // 1. Fetch vuln
        ...
        if (process.env.GITHUBTOKEN) {
            try {
                ...
                const prBody = [
                    ...
                    ``,
                    `### Test suggestions`,
                    ...patchResult.test_suggestions.map((t) => `- ${t}`),
                    ``,
                    `---`,
    ...
    app.post("/api/patches/logistics/push", async (_req, reply) => {
        ...
        if (process.env.GITHUBTOKEN) {
            try {
                ...
                const treeItems = await Promise.all(files.map(async ({ filename, content }) => {
    ...
    app.post("/api/patches/:id/approve", async (req, reply) => {
    ...
    app.post("/api/patches/:id/merge", async (req, reply) => {
        const { data: patch, error } = await supabase
            .from("patch_records")
            ...
            try {
                ...
                if (prNumber) {
                    await octokit.pulls.merge({
                        owner, repo, pull_number: prNumber,
                        merge_method: "squash",
                        commit_title: `[Sentinel AI] Merge security patch — ${patch.vulnerabilities?.cve_id ?? patch.vuln_id}`,
                    });
                }
            }
            catch (ghErr) {
            ...
            try {
                switch (svc) {
                    ...
                    case "grafana": {
                        ...
                        if (anonRes.ok)
                            r.actions.push("org preferences updated");
                        break;
                    }
                    case "postgresql": {
    ...
    app.patch("/api/patches/:id", async (req, reply) => {
        const { data, error } = await supabase
            .from("patch_records")
            .update(req.body)
            .eq("patch_id", req.params.id)
...
// ── Helpers ──────────────────────────────────────────────────────────────────
function inferLanguage(vuln) {
    const tags = vuln.tags ?? [];
    const joined = [...tags, String(vuln.blast_radius ?? ""), String(vuln.scan_source ?? "")].join(" ").toLowerCase();
    if (joined.includes("java") || joined.includes("spring") || joined.includes("log4"))
        return "java";
    if (joined.includes("python") || joined.includes("flask") || joined.includes("django"))
        return "python";
    if (joined.includes("node") || joined.includes("express") || joined.includes("typescript"))
        return "typescript";
    if (joined.includes("php") || joined.includes("openemr"))
        return "php";
    if (joined.includes("ruby") || joined.includes("rails") || joined.includes("gitlab"))
        return "ruby";
    return "python";
}
function inferFramework(vuln) {
    const tags = vuln.tags ?? [];
    const joined = tags.join(" ").toLowerCase();
    if (joined.includes("spring"))
...
function inferFilePath(vuln) {
    const cve = String(vuln.cve_id ?? "unknown");
    const lang = inferLanguage(vuln);
    const ext = { java: "java", python: "py", typescript: "ts", php: "php", ruby: "rb" };
    return `src/security/patches/${cve.replace(/[^a-zA-Z0-9-]/g, "_")}.${ext[lang] ?? "py"}`;
}
function buildAffectedCodeTemplate(vuln) {
    const cwe = (vuln.cwe_ids ?? ["CWE-0"])[0];
    const lang = inferLanguage(vuln);
