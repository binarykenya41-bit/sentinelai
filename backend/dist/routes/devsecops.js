...
export async function devsecopsRoutes(app) {
    app.get("/api/devsecops/pipelines", async (req, reply) => {
    ...
    app.get("/api/devsecops/sbom", async (req, reply) => {
        let q = supabase.from("sbom_findings").select("*").order("severity");
        if (req.query.pipeline_id)
            q = q.eq("pipeline_id", req.query.pipeline_id);
