...
export async function endpointSecurityRoutes(app) {
    app.get("/api/endpoint-security", async (req, reply) => {
        ...
        if (req.query.tactic)
            q = q.eq("tactic", req.query.tactic);
    ...
    app.get("/api/endpoint-security/stats", async (req, reply) => {
        ...
        for (const r of rows) {
            if (r.technique_id)
                techniqueCount[r.technique_id] = (techniqueCount[r.technique_id] ?? 0) + 1;
        }
        ...
        if (industry && industry !== 'all')
            endpointsQ = endpointsQ.eq("industry", industry);
        const { data: endpoints } = await endpointsQ;
        const endpointMap = {};
        for (const r of (endpoints ?? [])) {
            if (!endpointMap[r.endpoint])
                endpointMap[r.endpoint] = { hostname: r.hostname, os: r.os, alerts: 0, compliant: true };
            endpointMap[r.endpoint].alerts++;
            if (r.status === "Active")
                endpointMap[r.endpoint].compliant = false;
        }
        return reply.send({
            ...
            by_tactic: rows.reduce((acc, r) => { if (r.tactic) {
    ...
    app.get("/api/endpoint-security/endpoints", async (_req, reply) => {
        ...
        for (const r of (data ?? [])) {
            if (!endpointMap[r.endpoint])
                endpointMap[r.endpoint] = { hostname: r.hostname, os: r.os, alerts: 0, active_alerts: 0, last_alert: r.detected_at };
            endpointMap[r.endpoint].alerts++;
            if (r.status === "Active" || r.status === "Investigating")
                endpointMap[r.endpoint].active_alerts++;
            if (r.detected_at > endpointMap[r.endpoint].last_alert)
                endpointMap[r.endpoint].last_alert = r.detected_at;
        }
        return reply.send(Object.entries(endpointMap).map(([id, ep]) => ({ id, ...ep })));
    });
    app.get("/api/endpoint-security/:id", async (req, reply) => {
        const { data, error } = await supabase.from("edr_alerts").select("*").eq("alert_id", req.params.id).single();
        if (error)
            return reply.status(404).send({ error: "Alert not found" });
        return reply.send(data);
    });
    app.patch("/api/endpoint-security/:id", async (req, reply) => {
...
//# sourceMappingURL=endpoint-security.js.map