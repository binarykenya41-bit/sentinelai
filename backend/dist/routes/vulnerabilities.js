...
export async function vulnerabilitiesRoutes(app) {
    // GET /api/vulnerabilities — paginated list with rich filtering
    app.get("/api/vulnerabilities", async (req, reply) => {
        let query = supabase
        ...
        if (req.query.status)
            query = query.eq("remediation_status", req.query.status);
        if (req.query.kev === "true")
            query = query.eq("kev_status", true);
        if (req.query.min_cvss)
            query = query.gte("cvss_v3", parseFloat(req.query.min_cvss));
        if (req.query.max_cvss)
            query = query.lte("cvss_v3", parseFloat(req.query.max_cvss));
        if (req.query.scan_source)
            query = query.eq("scan_source", req.query.scan_source);
        if (req.query.cve)
            query = query.ilike("cve_id", `%${req.query.cve}%`);
    ...
    app.get("/api/vulnerabilities/stats", async (_req, reply) => {
        ...
        for (const v of vulns) {
            ...
            if (v.scan_source)
                sourceCount[v.scan_source] = (sourceCount[v.scan_source] ?? 0) + 1;
        }
        return reply.send({
            total: vulns.length,
    ...
    app.get("/api/vulnerabilities/:id", async (req, reply) => {
    ...
    app.patch("/api/vulnerabilities/:id/status", async (req, reply) => {
