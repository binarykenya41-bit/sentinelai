...
import { getModuleById, getModulesForCve, MODULE_CATALOG, } from "./modules.js";
// In-memory running simulations map (sandbox_id → status)
const runningSimulations = new Map();
function computeConfidence(module, exitCode, timedOut, stdout) {
    if (timedOut)
...
export async function runSimulation(req) {
    ...
    const { success, confidence } = computeConfidence(module, sandboxResult.exit_code, sandboxResult.timed_out, sandboxResult.stdout);
...
export async function killSimulation(sandboxId) {
...
export async function killAllSimulations() {
...
export function getRunningSimulations() {
